...
\@x{}\moduleLeftDash\@xx{ {\MODULE} Harmony}\moduleRightDash\@xx{}%
\@x{ {\EXTENDS} Integers ,\, Sequences ,\, FiniteSets}%
\@pvspace{8.0pt}%
\@x{ {\VARIABLE} CTXBAG ,\, SHARED ,\, FAILEDASSERT}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@pvspace{8.0pt}%
\@x{ Neg \.{\defeq} [ F \.{\mapsto}\@w{T} ,\, T \.{\mapsto}\@w{F} ]}%
\@x{}%
\@y{\@s{0}%
 add \ensuremath{var} with \ensuremath{val} to map
}%
\@xx{}%
 \@x{ NMap ( var ,\, val ,\, map )\@s{4.1} \.{\defeq} [ x \.{\in} ( (
 {\DOMAIN} map ) \.{\cup} \{ var \} ) \.{\,\backslash\,} \{\@w{FALSE} \}
 \.{\mapsto} {\IF} x \.{=} var \.{\THEN} val \.{\ELSE} map [ x ] ]}%
\@x{}%
\@y{\@s{0}%
 remove \ensuremath{var} from map, until empty map, \ensuremath{i.e}.,
 \ensuremath{{\FALSE} \.{\mapsto} {\FALSE}
}}%
\@xx{}%
 \@x{ NMap2 ( var ,\, map ) \.{\defeq} [ x \.{\in} ( ( {\DOMAIN} map )
 \.{\,\backslash\,} \{ var \} ) \.{\cup} \{\@w{FALSE} \} \.{\mapsto} {\IF} x
 \.{\in} {\DOMAIN} map \.{\THEN} map [ x ] \.{\ELSE} {\FALSE} ]}%
\@x{}%
\@y{\@s{0}%
 remove \ensuremath{var} from map
}%
\@xx{}%
 \@x{ NMapReturn ( var ,\, map ) \.{\defeq} [ x \.{\in} ( ( {\DOMAIN} map )
 \.{\,\backslash\,} \{ var \} ) \.{\mapsto} map [ x ] ]}%
\@x{ {\RECURSIVE} NTail ( \_ ,\, \_ )}%
\@x{ {\RECURSIVE} NHead ( \_ ,\, \_ )}%
\@x{ {\RECURSIVE} AddMult ( \_ ,\, \_ ,\, \_ )}%
 \@x{ AddMult ( var\_tup ,\, val\_tup ,\, map ) \.{\defeq} {\IF} Len (
 var\_tup ) \.{=} 1 \.{\THEN} [ x \.{\in} ( ( {\DOMAIN} map ) \.{\cup} \{
 Head ( var\_tup ) \} ) \.{\,\backslash\,} \{\@w{FALSE} \} \.{\mapsto} {\IF}
 x \.{=} Head ( var\_tup ) \.{\THEN} Head ( val\_tup ) \.{\ELSE} map [ x ] ]}%
 \@x{\@s{162.38} \.{\ELSE} [ x \.{\in} ( ( {\DOMAIN} AddMult ( Tail ( var\_tup
 ) ,\, Tail ( val\_tup ) ,\, map ) ) \.{\cup} \{ Head ( var\_tup ) \} )
 \.{\,\backslash\,} \{\@w{FALSE} \} \.{\mapsto} {\IF} x \.{=} Head ( var\_tup
 )\@s{6.71} \.{\THEN} Head ( val\_tup ) \.{\ELSE} AddMult ( Tail ( var\_tup )
 ,\, Tail ( val\_tup ) ,\, map ) [ x ] ]}%
\@pvspace{16.0pt}%
\@x{}%
\@y{\@s{0}%
 the last \ensuremath{n} elements of the list
}%
\@xx{}%
 \@x{ NTail ( n ,\, tup )\@s{20.5} \.{\defeq} {\IF} n \.{=} 1 \.{\THEN} Tail (
 tup ) \.{\ELSE} NTail ( n \.{-} 1 ,\, Tail ( tup ) )}%
\@x{}%
\@y{\@s{0}%
 the first \ensuremath{n} elements of a \ensuremath{tup
}}%
\@xx{}%
 \@x{ NHead ( n ,\, tup )\@s{20.5} \.{\defeq} {\IF} n \.{=} 1 \.{\THEN}
 {\langle} Head ( tup ) {\rangle} \.{\ELSE} NHead ( n \.{-} 1 ,\, Tail ( tup
 ) ) \.{\circ} {\langle} Head ( tup ) {\rangle}}%
\@x{}%
\@y{\@s{0}%
 nth element of a \ensuremath{tup
}}%
\@xx{}%
 \@x{ SpawnHead ( ctx )\@s{12.29} \.{\defeq} NHead ( 3 ,\, CTXBAG [ ctx ] .
 stack )}%
 \@x{ SpawnTail ( ctx )\@s{16.92} \.{\defeq} NTail ( 3 ,\, CTXBAG [ ctx ] .
 stack )}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 Number of contexts with specified \ensuremath{PC
}}%
\@xx{}%
 \@x{ countLabel ( This\_PC ) \.{\defeq} Cardinality ( \{ x \.{\in} {\DOMAIN}
 CTXBAG \.{:} CTXBAG [ x ] . pc \.{=} This\_PC \} )}%
\@pvspace{8.0pt}%
\@x{ DefaultStateCheckPartial ( ctx ,\, PC ) \.{\defeq}}%
 \@x{\@s{8.2} \.{\land} ( CTXBAG [ ctx ] . atomic \.{=} {\TRUE} \.{\lor} (
 \forall\, x \.{\in} {\DOMAIN} CTXBAG \.{:} CTXBAG [ x ] . atomic \.{=}
 {\FALSE} ) )}%
\@x{\@s{8.2} \.{\land} CTXBAG [ ctx ] . pc \.{=} PC}%
\@x{\@s{8.2} \.{\land} CTXBAG [ ctx ] . active \.{=} {\TRUE}}%
\@x{ DefaultStateCheck ( ctx ,\, PC ) \.{\defeq}}%
\@x{\@s{8.2} \.{\land} DefaultStateCheckPartial ( ctx ,\, PC )}%
\@x{\@s{8.2} \.{\land} {\UNCHANGED} SHARED}%
\@x{\@s{8.2} \.{\land} {\UNCHANGED} FAILEDASSERT}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 empty record
...
\@xx{}%
\@x{ e\_rec \.{\defeq} [ {\FALSE} \.{\mapsto} {\FALSE} ]}%
\@x{}%
\@y{\@s{0}%
 a new context
}%
\@xx{}%
 \@x{ new\_ctx \.{\defeq} [ pc \.{\mapsto} 0 ,\, stack \.{\mapsto} {\langle}
 {\langle} {\rangle} {\rangle} ,\, vars \.{\mapsto} e\_rec ,\, active
 \.{\mapsto} {\FALSE} ,\, spn \.{\mapsto} {\FALSE} ,\, atomic \.{\mapsto}
 {\FALSE} ]}%
\begin{lcom}{2.5}%
...
\end{cpar}%
\end{lcom}%
 \@x{ init\_ctx \.{\defeq} [ pc \.{\mapsto} 0 ,\, stack \.{\mapsto} {\langle}
 {\langle} {\rangle} {\rangle} ,\, vars \.{\mapsto} e\_rec ,\, active
 \.{\mapsto} {\TRUE} ,\, spn \.{\mapsto} {\TRUE} ,\, atomic \.{\mapsto}
 {\FALSE} ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{\@s{4.1} \.{\land} SHARED\@s{1.51} \.{=} e\_rec}%
\@y{\@s{0}%
 start empty
}%
\@xx{}%
\@x{\@s{4.1} \.{\land} CTXBAG \.{=} [ c0 \.{\mapsto} init\_ctx ,\,}%
\@x{\@s{75.89} c1 \.{\mapsto} new\_ctx ,\,}%
\@x{\@s{75.89} c2 \.{\mapsto} new\_ctx ]}%
\@x{\@s{4.1} \.{\land} FAILEDASSERT \.{=} {\FALSE}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 push \ensuremath{val} onto head of \ensuremath{ctx} stack
}%
\@xx{}%
\@x{ Push ( ctx ,\, PC ,\, val ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land}\@s{6.90} DefaultStateCheck ( ctx ,\, PC )}%
\@x{\@s{4.1} \.{\land}\@s{6.90} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT}}%
\@x{\@s{97.90} {\bang} [ ctx ] . pc \.{=} PC \.{+} 1 ,\,}%
 \@x{\@s{97.90} {\bang} [ ctx ] . stack \.{=} {\langle} val {\rangle}
 \.{\circ} CTXBAG [ ctx ] . stack ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 thread store
...
\@xx{}%
\@x{ StoreVar ( ctx ,\, PC ,\, var ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land} DefaultStateCheck ( ctx ,\, PC )}%
\@x{\@s{4.1} \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT}}%
\@x{\@s{90.99} {\bang} [ ctx ] . pc \.{=} PC \.{+} 1 ,\,}%
 \@x{\@s{90.99} {\bang} [ ctx ] . stack \.{=} Tail ( CTXBAG [ ctx ] . stack )
 ,\,}%
\@x{\@s{90.99} {\bang} [ ctx ] . vars \.{=} NMap ( var ,\,}%
\@x{\@s{180.97} Head ( CTXBAG [ ctx ] . stack ) ,\,}%
\@x{\@s{180.97} CTXBAG [ ctx ] . vars )}%
\@x{\@s{75.91} ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{ Store ( ctx ,\, PC ,\, var ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land} DefaultStateCheckPartial ( ctx ,\, PC )}%
\@x{\@s{4.1} \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT}}%
\@x{\@s{90.99} {\bang} [ ctx ] . pc \.{=} PC \.{+} 1 ,\,}%
 \@x{\@s{90.99} {\bang} [ ctx ] . stack \.{=} Tail ( CTXBAG [ ctx ] . stack )
 ]}%
\@x{\@s{4.1} \.{\land} SHARED \.{'}\@s{1.51} \.{=} {\IF} var \.{=}\@w{}}%
\@x{\@s{75.91} \.{\THEN} NMap (}%
\@x{\@s{92.31} Head ( Tail ( Tail ( CTXBAG [ ctx ] . stack ) ) ) ,\,}%
\@x{\@s{92.31} NMap ( Head ( Tail ( CTXBAG [ ctx ] . stack ) ) ,\,}%
\@x{\@s{123.46} Head ( CTXBAG [ ctx ] . stack ) ,\,}%
\@x{\@s{123.46} SHARED [ Head ( Tail ( Tail ( CTXBAG [ ctx ] . stack ) ) ) ]}%
\@x{\@s{92.31} ) ,\,}%
\@x{\@s{92.31} SHARED}%
\@x{\@s{75.91} )}%
\@x{\@s{75.91} \.{\ELSE} NMap (}%
\@x{\@s{92.31} var ,\,}%
\@x{\@s{92.31} Head ( CTXBAG [ ctx ] . stack ) ,\,}%
\@x{\@s{92.31} SHARED}%
\@x{\@s{75.91} )}%
\@x{\@s{4.1} \.{\land} {\UNCHANGED} FAILEDASSERT}%
\@pvspace{8.0pt}%
\@x{ Jump ( ctx ,\, PC ,\, PC\_new ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land}\@s{9.32} DefaultStateCheck ( ctx ,\, PC )}%
 \@x{\@s{4.1} \.{\land}\@s{9.32} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT} {\bang}
 [ ctx ] . pc \.{=} PC\_new ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{ Load ( ctx ,\, PC ,\, var\_name ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land}\@s{6.40} DefaultStateCheck ( ctx ,\, PC )}%
\@x{\@s{4.1}}%
\@y{\@s{0}%
 push the value of a shared variable onto the stack
}%
\@xx{}%
\@x{\@s{4.1} \.{\land} CTXBAG \.{'} \.{=} {\IF} var\_name \.{=}\@w{}}%
\@x{\@s{75.91} \.{\THEN} [ CTXBAG {\EXCEPT}}%
\@x{\@s{118.20} {\bang} [ ctx ] . pc \.{=} PC \.{+} 1 ,\,}%
 \@x{\@s{118.20} {\bang} [ ctx ] . stack \.{=} {\langle} SHARED [ Head ( Tail
 ( CTXBAG [ ctx ] . stack ) ) ] [ Head ( CTXBAG [ ctx ] . stack ) ] {\rangle}
 \.{\circ} CTXBAG [ ctx ] . stack ]}%
\@x{\@s{75.91} \.{\ELSE} [ CTXBAG {\EXCEPT}}%
\@x{\@s{118.20} {\bang} [ ctx ] . pc \.{=} PC \.{+} 1 ,\,}%
 \@x{\@s{118.20} {\bang} [ ctx ] . stack \.{=} {\langle} SHARED [ var\_name ]
 {\rangle} \.{\circ} CTXBAG [ ctx ] . stack ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{ LoadVar ( ctx ,\, PC ,\, var\_name ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land} DefaultStateCheck ( ctx ,\, PC )}%
\@x{\@s{4.1} \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT}}%
\@x{\@s{90.99} {\bang} [ ctx ] . pc \.{=} PC \.{+} 1 ,\,}%
 \@x{\@s{90.99} {\bang} [ ctx ] . stack \.{=} {\langle} CTXBAG [ ctx ] . vars
 [ var\_name ] {\rangle} \.{\circ} CTXBAG [ ctx ] . stack ]}%
\@pvspace{8.0pt}%
\@x{ Spawn ( ctxa ,\, PC ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land} DefaultStateCheck ( ctxa ,\, PC )}%
\@x{\@s{4.1} \.{\land} \.{\LET} SpStk \.{\defeq} SpawnHead ( ctxa ) \.{\IN}}%
 \@x{\@s{15.21} \.{\LET} ctxb \.{\defeq} {\CHOOSE} x \.{\in} {\DOMAIN} CTXBAG
 \.{:} CTXBAG [ x ] . active \.{=} {\FALSE} \.{\IN}}%
\@x{\@s{15.21} \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT}}%
\@x{\@s{98.00} {\bang} [ ctxa ] . pc \.{=} PC \.{+} 1 ,\,}%
\@x{\@s{98.00} {\bang} [ ctxa ] . stack \.{=} SpawnTail ( ctxa ) ,\,}%
\@x{\@s{98.00} {\bang} [ ctxb ] . pc \.{=} Head ( SpStk ) ,\,}%
\@x{\@s{98.00} {\bang} [ ctxb ] . stack \.{=} Tail ( SpStk ) ,\,}%
\@x{\@s{98.00} {\bang} [ ctxb ] . active \.{=} {\TRUE} ,\,}%
\@x{\@s{98.00} {\bang} [ ctxb ] . spn \.{=} {\TRUE} ]}%
\@pvspace{8.0pt}%
\@x{\@s{4.1}}%
\@y{\@s{0}%
 delete thread variable \ensuremath{var
}}%
\@xx{}%
\@x{ DelVar ( ctx ,\, PC ,\, var ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land} DefaultStateCheck ( ctx ,\, PC )}%
\@x{\@s{4.1} \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT}}%
\@x{\@s{90.99} {\bang} [ ctx ] . pc \.{=} PC \.{+} 1 ,\,}%
 \@x{\@s{90.99} {\bang} [ ctx ] . vars \.{=} NMap2 ( var ,\, CTXBAG [ ctx ] .
 vars ) ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 take top of the context\mbox{'}s stack and assign it to \ensuremath{Frame}
 instruction arguments \ensuremath{args
}}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 \ensuremath{TODO} want to do store \ensuremath{var} on possibly a tuple,
 only works for single \ensuremath{var} now
}%
\@xx{}%
\@x{ Frame ( ctx ,\, PC ,\, args ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land} DefaultStateCheck ( ctx ,\, PC )}%
\@x{\@s{4.1} \.{\land} CTXBAG [ ctx ] . spn \.{=} {\TRUE}}%
\@x{\@s{4.1} \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT}}%
\@x{\@s{90.99} {\bang} [ ctx ] . pc \.{=} PC \.{+} 1 ,\,}%
 \@x{\@s{90.99} {\bang} [ ctx ] . stack \.{=} Tail ( CTXBAG [ ctx ] . stack )
 ,\,}%
 \@x{\@s{90.99} {\bang} [ ctx ] . vars \.{=} AddMult ( args ,\, CTXBAG [ ctx ]
 . stack ,\, CTXBAG [ ctx ] . vars ) ]}%
\@pvspace{8.0pt}%
\@x{ Return ( ctx ,\, PC ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land} DefaultStateCheck ( ctx ,\, PC )}%
\@x{\@s{4.1} \.{\land} {\IF} CTXBAG [ ctx ] . spn \.{=} {\TRUE}}%
\@x{\@s{15.21} \.{\THEN}}%
 \@x{\@s{15.21} \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT} {\bang} [ ctx
 ] . active \.{=} {\FALSE} ]}%
\@x{\@s{15.21} \.{\ELSE}}%
\@x{\@s{15.21} \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT}}%
 \@x{\@s{98.00} {\bang} [ ctx ] . pc \.{=} Head ( CTXBAG [ ctx ] . stack )
 ,\,}%
 \@x{\@s{98.00} {\bang} [ ctx ] . stack \.{=} Tail ( CTXBAG [ ctx ] . stack )
 ]}%
\@pvspace{8.0pt}%
\@x{ AssertH ( ctx ,\, PC ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land} DefaultStateCheckPartial ( ctx ,\, PC )}%
\@x{\@s{4.1} \.{\land} {\UNCHANGED} SHARED}%
\@x{\@s{4.1} \.{\land} {\IF} Head ( CTXBAG [ ctx ] . stack ) \.{=} {\TRUE}}%
\@x{\@s{15.21} \.{\THEN}}%
\@x{\@s{15.21} ( \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT}}%
\@x{\@s{105.99} {\bang} [ ctx ] . pc \.{=} PC \.{+} 1 ,\,}%
 \@x{\@s{105.99} {\bang} [ ctx ] . stack \.{=} Tail ( CTXBAG [ ctx ] . stack )
 ]}%
\@x{\@s{19.31} \.{\land} {\UNCHANGED} FAILEDASSERT )}%
\@x{\@s{15.21} \.{\ELSE}}%
 \@x{\@s{15.21} ( \.{\land} CTXBAG \.{'} \.{=} [ x \.{\in} {\DOMAIN} CTXBAG
 \.{\mapsto}\@s{4.1} [}%
\@x{\@s{116.97} pc \.{\mapsto} CTXBAG [ x ] . pc ,\,}%
\@x{\@s{116.97} stack \.{\mapsto} CTXBAG [ x ] . stack ,\,}%
\@x{\@s{116.97} vars \.{\mapsto} CTXBAG [ x ] . vars ,\,}%
\@x{\@s{116.97} active \.{\mapsto} {\FALSE} ,\,}%
\@x{\@s{116.97} spn \.{\mapsto} CTXBAG [ x ] . spn ,\,}%
\@x{\@s{116.97} atomic \.{\mapsto} CTXBAG [ x ] . atomic}%
\@x{\@s{103.63} ]}%
\@x{\@s{90.91} ]}%
\@x{\@s{19.31} \.{\land} FAILEDASSERT \.{'} \.{=} {\TRUE} )}%
\@pvspace{8.0pt}%
\@x{ JumpCond ( ctx ,\, PC ,\, exp ,\, PC\_new ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land} DefaultStateCheck ( ctx ,\, PC )}%
 \@x{\@s{4.1} \.{\land} {\IF} Head ( CTXBAG [ ctx ] . stack ) \.{=} exp
 \.{\THEN}}%
 \@x{\@s{15.21} ( \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT} {\bang} [
 ctx ] . pc \.{=} PC\_new ,\, {\bang} [ ctx ] . stack \.{=} Tail ( CTXBAG [
 ctx ] . stack ) ] )}%
\@x{\@s{15.21} \.{\ELSE}}%
 \@x{\@s{15.21} ( \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT} {\bang} [
 ctx ] . pc \.{=} PC \.{+} 1 ,\, {\bang} [ ctx ] . stack \.{=} Tail ( CTXBAG
 [ ctx ] . stack ) ] )}%
\@pvspace{8.0pt}%
\@x{ AtomicInc ( ctx ,\, PC ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land} DefaultStateCheck ( ctx ,\, PC )}%
\@x{\@s{4.1} \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT}}%
\@x{\@s{90.99} {\bang} [ ctx ] . pc \.{=} PC \.{+} 1 ,\,}%
\@x{\@s{90.99} {\bang} [ ctx ] . atomic \.{=} {\TRUE} ]}%
\@pvspace{8.0pt}%
\@x{\@s{4.1} AtomicDec ( ctx ,\, PC ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land} DefaultStateCheck ( ctx ,\, PC )}%
\@x{\@s{4.1} \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT}}%
\@x{\@s{90.99} {\bang} [ ctx ] . pc \.{=} PC \.{+} 1 ,\,}%
\@x{\@s{90.99} {\bang} [ ctx ] . atomic \.{=} {\FALSE} ]}%
\@pvspace{8.0pt}%
\@x{ NotOp ( ctx ,\, PC ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land} DefaultStateCheck ( ctx ,\, PC )}%
\@x{\@s{4.1} \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT}}%
\@x{\@s{90.99} {\bang} [ ctx ] . pc \.{=} PC \.{+} 1 ,\,}%
 \@x{\@s{90.99} {\bang} [ ctx ] . stack \.{=} {\langle} Neg [ Head ( CTXBAG [
 ctx ] . stack ) ] {\rangle} \.{\circ} Tail ( CTXBAG [ ctx ] . stack ) ]}%
\@pvspace{8.0pt}%
\@x{ EqOp ( ctx ,\, PC ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land}\@s{9.58} DefaultStateCheck ( ctx ,\, PC )}%
\@x{\@s{4.1} \.{\land}\@s{9.58} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT}}%
\@x{\@s{100.57} {\bang} [ ctx ] . pc \.{=} PC \.{+} 1 ,\,}%
 \@x{\@s{100.57} {\bang} [ ctx ] . stack \.{=} {\langle} ( Head ( CTXBAG [ ctx
 ] . stack ) \.{=} Head ( Tail ( CTXBAG [ ctx ] . stack ) ) ) {\rangle}
 \.{\circ} Tail ( CTXBAG [ ctx ] . stack ) ]}%
\@pvspace{8.0pt}%
\@x{ Dummy ( ctx ,\, PC ) \.{\defeq}}%
\@x{\@s{4.1} \.{\land} DefaultStateCheck ( ctx ,\, PC )}%
 \@x{\@s{4.1} \.{\land} CTXBAG \.{'} \.{=} [ CTXBAG {\EXCEPT} {\bang} [ ctx ]
 . pc \.{=} PC \.{+} 1 ]}%
\@x{}\bottombar\@xx{}%
\setboolean{shading}{false}
\begin{lcom}{0}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Dec} 10 21:18:32 \ensuremath{EST} 2021 by \ensuremath{katyblumer
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Last modified \ensuremath{Fri}
 \ensuremath{Dec} 10 19:54:36 \ensuremath{EST} 2021 by \ensuremath{noah
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Last modified \ensuremath{Thu}
 \ensuremath{Nov} 18 16:26:44 \ensuremath{EST} 2021 by
 \ensuremath{arielkellison
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Tue} \ensuremath{Nov} 02
 18:59:20 \ensuremath{EDT} 2021 by \ensuremath{arielkellison
}%
\end{cpar}%
\end{lcom}%
\end{document}
