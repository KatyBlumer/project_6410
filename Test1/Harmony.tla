...
Store(ctx,var,PC) ==
 ...
 /\ SHARED' = IF var = ""
              THEN NMap(  (*---- SHARED[stack.3] = ...that whole thing below ----*)
                      Head(Tail(Tail(CTXBAG[ctx].stack))), ) (* stack.3 *)
                      NMap( (*-- SHARED[stack.3][stack.2] = stack.1 --*)
                        Head(Tail(CTXBAG[ctx].stack)), (* stack.2 *)
                        Head(CTXBAG[ctx].stack), (* stack.1 *)
                        SHARED[Head(Tail(Tail(CTXBAG[ctx].stack)))]  (* SHARED[stack.3]? *)
                      ),
                      SHARED
                   )
              ELSE NMap(  (*---- SHARED[var] = stack.1 ----*)
                      var,
                      Head(CTXBAG[ctx].stack),
                      SHARED
                   )
...
Load(ctx,var_name,PC) ==
 ...
 /\ CTXBAG' = IF var_name = ""
              THEN [CTXBAG EXCEPT
                      ![ctx].pc = PC + 1,
                      ![ctx].stack = <<SHARED[Head(Tail(CTXBAG[ctx].stack))][Head(CTXBAG[ctx].stack)]>>  (* SHARED[stack.2][stack.1] *)
                                     \o CTXBAG[ctx].stack
                   ]
              ELSE [CTXBAG EXCEPT
                      ![ctx].pc = PC + 1,
                      ![ctx].stack = <<SHARED[var_name]>>
                                     \o CTXBAG[ctx].stack
                   ]
...
Spawn(ctxa,PC) ==  (* chooses one of the preinitialized contexts; assumes one is available? *)
 /\ CommonAssertsTODORenameMe(ctxa,PC)
